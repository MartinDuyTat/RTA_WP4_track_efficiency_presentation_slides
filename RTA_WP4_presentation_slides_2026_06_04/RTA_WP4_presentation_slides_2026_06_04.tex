...
\begin{frame}{2025 pre-TS Velo discrepancies}
  ...
  \begin{itemize}
    ...
    \item{Two consecutive modules that are not working properly can significant impact track efficiency!}
  \end{itemize}
  \begin{figure}[htb]
    \centering
...
\begin{frame}{2025 pre-TS Velo discrepancies}
  ...
  \end{itemize}
  \vspace{0.5cm}
  {When the larger MC samples are available, we propose the following mitigation: Change $p$--$\eta$ binning to $p$--$\eta$--$\phi$ with two $\phi$ bins corresponding to left and right ($\abs{\phi} < \pi/2$ and $\abs{\phi} \geq \pi/2$)}
\end{frame}
